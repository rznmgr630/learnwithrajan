  ...
  return (
    <main ref={readerRef} className="fixed inset-0 z-30 flex min-h-0 flex-col overflow-hidden bg-[var(--background)] px-4 py-5 sm:px-6">
      ...
      <div className="absolute right-4 top-5 z-10 flex flex-col gap-2 sm:right-6">
        ...
        <button type="button" onClick={() => setZoom((value) => Math.min(1.5, Number((value + 0.1).toFixed(1))))} disabled={zoom === 1.5} className="grid h-9 w-9 place-items-center rounded-lg border border-[var(--border)] bg-[var(--elevated)] text-[var(--text)] shadow-sm transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Zoom in" title="Zoom in">
        ...
        <button type="button" onClick={toggleFullscreen} className="grid h-9 w-9 place-items-center rounded-lg border border-[var(--border)] bg-[var(--elevated)] text-[var(--text)] shadow-sm transition hover:brightness-110" aria-label={isFullscreen ? "Exit full screen" : "Full screen"} title={isFullscreen ? "Exit full screen" : "Full screen"}>
