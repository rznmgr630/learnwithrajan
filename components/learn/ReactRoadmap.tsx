...
import { getAllReactRoadmapDays, REACT_TOTAL_DAYS } from "@/lib/react-learning/react-challenge-data";
...
export function ReactRoadmap() {
  ...
  const allDays = useMemo(() => getAllReactRoadmapDays(), []);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-8 sm:px-6">
      ...
      <ul className={`mt-8 ${dayGridClass}`} id="react-roadmap-days" aria-label={t("reactRoadmap.daysListAria")}>
        {allDays.map((d) => {
      ...
      <div className="mt-14 flex flex-col items-center gap-4" id="react-roadmap-bottom">
        <button
          type="button"
          ...
          onClick={() => document.getElementById("react-roadmap-blurb")?.scrollIntoView({ behavior: "smooth" })}
        >
        ...
        <p id="react-roadmap-blurb" className="max-w-2xl text-center text-sm leading-relaxed text-[var(--muted)]">
          {t("reactRoadmap.bottomBlurb")}
        </p>
      </div>
