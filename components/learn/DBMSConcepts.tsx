...
export function DBMSConcepts() {
  const [active, setActive] = useState<DBMSConcept | null>(null);
      ...
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-4xl px-4 pb-20 pt-12 sm:px-6">
          ...
          <div className="relative mb-10">
            ...
            <div className="relative">
              ...
              <p className="mt-4 max-w-xl text-[var(--muted)]">
                {DBMS_CONCEPT_COUNT} concepts — from what a database and a DBMS actually are,
                through data models, the ER model, relational database theory, all five SQL
                command categories, and advanced topics like normalization, indexing,
                transactions, ACID, and deadlocks.
              </p>
            </div>
          </div>
          ...
          <div className="mb-10 flex flex-wrap gap-4">
            {[
              { label: "Concepts", value: `${DBMS_CONCEPT_COUNT}` },
              { label: "Sections", value: `${DBMS_SECTIONS.length}` },
              { label: "Code Examples", value: `${DBMS_CONCEPT_COUNT}` },
            ].map((s) => (
              <div
                key={s.label}
          ...
          <div className="flex flex-col gap-8">
            {DBMS_SECTIONS.map((section) => {
              const sectionConcepts = DBMS_CONCEPTS.filter(
                (c) => c.section === section
              );
