...
import { JapaneseN3DayDetailPanel } from "@/components/learn/JapaneseN3DayDetailPanel";
import { JapaneseWeeklyTestPanel } from "@/components/learn/JapaneseWeeklyTestPanel";
import {
  JAPANESE_N3_FULL_LEVEL_MOCK,
  JAPANESE_N3_WEEKS,
  JP_N3_TOTAL_DAYS,
  resolveJapaneseN3WeeklyTestForRoadmap,
...
const weeklyTestIds = [
  ...JAPANESE_N3_WEEKS.filter((w) => w.weeklyTest).map((w) => w.id),
  JAPANESE_N3_FULL_LEVEL_MOCK.id,
];
...
function weeklyTestNeighbour(currentId: string | null, offset: -1 | 1, locale: Locale): LessonNavTarget | null {
  ...
  const test = id ? resolveJapaneseN3WeeklyTestForRoadmap(id) : undefined;
  return test ? { id, title: pickLocalized(test.title, locale) } : null;
}

export function JapaneseN3Roadmap() {
  ...
  const {
    ...
    toggleWeeklyTest,
    isWeeklyTestDone,
  } = useJapaneseN3Progress();
  const [openWeekId, setOpenWeekId] = useState<string | null>("jn3-w1");
  const [detailDay, setDetailDay] = useState<number | null>(null);
  const [weeklyTestWeekId, setWeeklyTestWeekId] = useState<string | null>(null);

  const barWidth = useMemo(
    () => `${Math.min(100, Math.round((completedCount / JP_N3_TOTAL_DAYS) * 100))}%`,
    [completedCount],
  );
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-8 sm:px-6">
      <div className="mb-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-[var(--text)] sm:text-3xl">{t("jpN3Roadmap.title")}</h1>
          <p className="mt-1 text-sm text-[var(--muted)]">{t("jpN3Roadmap.subtitle")}</p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <span className="text-sm text-[var(--muted)]">{t("jpN3Roadmap.overallProgress")}</span>
          <div className="flex items-center gap-3">
            <span className="text-sm tabular-nums text-[var(--text)]">
              {completedCount}/{JP_N3_TOTAL_DAYS} {t("jpN3Roadmap.days")}
            </span>
          </div>
        </div>
        ...
        <p className="mt-2 text-right text-xs text-[var(--muted)]">
          {percent}
          {t("jpN3Roadmap.percentComplete")}
        </p>
        <p className="mt-1 text-right text-xs text-[var(--faint)]">
          {t("jpN3Roadmap.weeklyTestsProgress")} {weeklyTestsCompletedCount}/{weeklyTestTotal} {t("jpN3Roadmap.markedDone")}
        </p>
      </div>

      <div className="mt-8 space-y-2" id="jp-n3-roadmap-weeks">
        {JAPANESE_N3_WEEKS.map((week) => {
            ...
            >
              ...
              >
                ...
                <span className="text-sm tabular-nums text-[var(--muted)]">
                  {doneInWeek}/{totalInWeek} {t("jpN3Roadmap.doneSlash")}
                </span>
                <svg
                      ...
                      <div className="flex flex-col gap-4 rounded-xl border border-[var(--border)] bg-[color-mix(in_oklab,var(--elevated)_35%,transparent)] p-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0 flex-1">
                          <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--muted)]">
                            {t("jpN3Roadmap.weeklyRecapLabel")}
                          </p>
                          ...
                          <p className="mt-2 text-[11px] text-[var(--faint)]">
                            {wt.subTests?.length
                              ? t("jpN3Roadmap.fivePapersBlurb")
                              : tParams("jpN3Roadmap.singlePaperBlurb", {
                        ...
                        <div className="flex shrink-0 flex-col gap-2 sm:items-end">
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-[var(--muted)]">{t("jpN3Roadmap.done")}</span>
                            <button
                              type="button"
                              role="checkbox"
      ...
      <div className="mt-10" id="jp-n3-roadmap-full-mock">
        <div className="rounded-2xl border border-[var(--border)] bg-[color-mix(in_oklab,var(--elevated)_35%,transparent)] p-5 shadow-xl sm:p-6">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--muted)]">{t("jpN3Roadmap.courseFinale")}</p>
          <h3 className="mt-2 text-lg font-semibold leading-snug text-[var(--text)]">
            <RichText text={pickLocalized(JAPANESE_N3_FULL_LEVEL_MOCK.title, locale)} />
          </h3>
          <p className="mt-1 text-sm leading-relaxed text-[var(--muted)]">
            <RichText text={pickLocalized(JAPANESE_N3_FULL_LEVEL_MOCK.subtitle, locale)} />
          </p>
          <p className="mt-3 text-[11px] leading-relaxed text-[var(--faint)]">
            {tParams("jpN3Roadmap.fullMockBlurb", {
              from: JAPANESE_N3_FULL_LEVEL_MOCK.coversDayRange[0],
              to: JAPANESE_N3_FULL_LEVEL_MOCK.coversDayRange[1],
            })}
          </p>
          <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                role="checkbox"
                aria-checked={isWeeklyTestDone(JAPANESE_N3_FULL_LEVEL_MOCK.id)}
                onClick={() => toggleWeeklyTest(JAPANESE_N3_FULL_LEVEL_MOCK.id)}
                className={[
                  "grid h-8 w-8 shrink-0 place-items-center rounded-lg border transition",
                  isWeeklyTestDone(JAPANESE_N3_FULL_LEVEL_MOCK.id)
                    ? "border-[var(--accent)] bg-[var(--accent)] text-[var(--accent-fg)]"
                    : "border-[var(--border)] bg-[var(--elevated)] hover:border-[var(--accent)]",
                ].join(" ")}
                aria-label={
      ...
      <JapaneseWeeklyTestPanel
        key={weeklyTestWeekId === null ? "wt-closed" : `wt-n3-${weeklyTestWeekId}`}
        test={
          weeklyTestWeekId === null
            ? null
            : (resolveJapaneseN3WeeklyTestForRoadmap(weeklyTestWeekId) ?? null)
        }
