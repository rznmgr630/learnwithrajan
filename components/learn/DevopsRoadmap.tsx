...
import { DEVOPS_ROADMAP_WEEKS, DEVOPS_TOTAL_DAYS } from "@/lib/devops-learning/devops-challenge-data";
    ...
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-8 sm:px-6">
      <div className="mb-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-[var(--text)] sm:text-3xl">
            {t("devopsRoadmap.title")}
          </h1>
          <p className="mt-1 text-sm text-[var(--muted)]">{t("devopsRoadmap.subtitle")}</p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <span className="text-sm text-[var(--muted)]">{t("devopsRoadmap.overallProgress")}</span>
          <span className="text-sm tabular-nums text-[var(--text)]">
            {completedCount}/{DEVOPS_TOTAL_DAYS} {t("hub.backend.days")}
          </span>
        </div>
        ...
        <p className="mt-2 text-right text-xs text-[var(--muted)]">
          {percent}
          {t("devopsRoadmap.percentComplete")}
        </p>
      </div>

      <div className="mt-8 space-y-2" id="devops-roadmap-weeks">
        {DEVOPS_ROADMAP_WEEKS.map((week) => {
