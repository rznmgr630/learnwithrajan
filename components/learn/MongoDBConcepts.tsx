...
import { MONGO_CONCEPTS, MONGO_CONCEPT_COUNT, MONGO_SECTIONS, type MongoConcept } from "@/lib/mongodb/concepts";
          ...
          <section>
            <h3 className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">
              <svg className="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M4.25 2A2.25 2.25 0 0 0 2 4.25v11.5A2.25 2.25 0 0 0 4.25 18h11.5A2.25 2.25 0 0 0 18 15.75V4.25A2.25 2.25 0 0 0 15.75 2H4.25ZM6.5 9.25a.75.75 0 0 0 0 1.5h2.69l-1.22 1.22a.75.75 0 1 0 1.06 1.06l2.5-2.5a.75.75 0 0 0 0-1.06l-2.5-2.5a.75.75 0 0 0-1.06 1.06l1.22 1.22H6.5Zm4.44-1.06a.75.75 0 0 1 1.06-1.06l2.5 2.5a.75.75 0 0 1 0 1.06l-2.5 2.5a.75.75 0 1 1-1.06-1.06l1.22-1.22H13.5a.75.75 0 0 1 0-1.5h-1.34l-1.22-1.22Z" clipRule="evenodd" />
              </svg>
              Example
            </h3>
            <pre className="rounded-lg bg-[color-mix(in_oklab,var(--elevated)_80%,transparent)] border border-[var(--border)] p-3 font-mono text-xs leading-relaxed text-[var(--text)] overflow-x-auto whitespace-pre-wrap">
              {concept.example.split("\n\n").map((block, i, blocksArr) => (
                <span key={i}>
                  {block.split("\n").map((line, j, linesArr) => (
                    <span key={j}>{line}{j < linesArr.length - 1 && "\n"}</span>
                  ))}
                  {i < blocksArr.length - 1 && "\n\n"}
                </span>
              ))}
            </pre>
          </section>
...
function ConceptCard({ concept, onClick }: { concept: MongoConcept; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
...
const orderedConcepts = MONGO_SECTIONS.flatMap((section) =>
  MONGO_CONCEPTS.filter((c) => c.section === section),
);

function neighbourOf(concept: MongoConcept | null, offset: -1 | 1): LessonNavTarget | null {
  if (!concept) return null;
...
export function MongoDBConcepts() {
  const [active, setActive] = useState<MongoConcept | null>(null);
      ...
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-4xl px-4 pb-20 pt-12 sm:px-6">
          ...
          <div className="relative mb-10">
            ...
            <div className="relative">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[var(--accent)]/20 bg-[var(--accent)]/8 px-3 py-1">
                ...
                <span className="text-xs font-medium text-[var(--accent)]">NoSQL · Database</span>
              </div>
              <h1 className="text-4xl font-bold tracking-tight text-[var(--text)] sm:text-5xl">
                MongoDB
                <br />
                <span className="text-[var(--accent)]">NoSQL Essentials</span>
              </h1>
              <p className="mt-4 max-w-xl text-[var(--muted)]">
                {MONGO_CONCEPT_COUNT} concepts — from documents and CRUD to aggregation pipelines, indexing,
          ...
          <div className="mb-10 flex flex-wrap gap-4">
            {[
              { label: "Concepts", value: `${MONGO_CONCEPT_COUNT}` },
              { label: "Sections", value: `${MONGO_SECTIONS.length}` },
              { label: "Examples", value: `${MONGO_CONCEPT_COUNT}` },
            ].map((s) => (
              <div
                key={s.label}
          ...
          <div className="flex flex-col gap-8">
            {MONGO_SECTIONS.map((section) => {
              const sectionConcepts = MONGO_CONCEPTS.filter((c) => c.section === section);
