"use client";

import Link from "next/link";
import { useId } from "react";
import { useLocale } from "@/components/i18n/LocaleProvider";
import { learnHubCardClass } from "@/components/learn/learn-hub-card-class";
import { PinButton } from "@/components/learn/PinButton";
import { HubAccordionSection } from "@/components/learn/HubAccordionSection";

export function LoksewaTracks() {
  const { t } = useLocale();
  const baseId = useId();

  return (
    <div className="flex flex-col gap-3">
      <HubAccordionSection
        sectionId={`${baseId}-general`}
        titleKey="hub.loksewa.groupGeneral"
        hintKey="hub.loksewa.groupGeneralHint"
        defaultOpen
      >
        <Link href="/learn/loksewa/gk" className={learnHubCardClass}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold tracking-tight text-[var(--text)] group-hover:text-[var(--accent)]">
                {t("hub.loksewaGK.title")}
              </h3>
              <p className="mt-1 text-sm text-[var(--muted)]">{t("hub.loksewaGK.subtitle")}</p>
            </div>
            <PinButton id="loksewa-gk" />
          </div>
          <span className="mt-6 block text-sm font-medium text-[var(--accent)] transition group-hover:brightness-110">
            {t("hub.loksewaGK.cta")}
          </span>
        </Link>

        <Link href="/learn/loksewa/geography" className={learnHubCardClass}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold tracking-tight text-[var(--text)] group-hover:text-[var(--accent)]">
                {t("hub.loksewaGeography.title")}
              </h3>
              <p className="mt-1 text-sm text-[var(--muted)]">{t("hub.loksewaGeography.subtitle")}</p>
            </div>
            <PinButton id="loksewa-geography" />
          </div>
          <span className="mt-6 block text-sm font-medium text-[var(--accent)] transition group-hover:brightness-110">
            {t("hub.loksewaGeography.cta")}
          </span>
        </Link>

        <Link href="/learn/loksewa/products" className={learnHubCardClass}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold tracking-tight text-[var(--text)] group-hover:text-[var(--accent)]">
                {t("hub.loksewaProducts.title")}
              </h3>
              <p className="mt-1 text-sm text-[var(--muted)]">{t("hub.loksewaProducts.subtitle")}</p>
            </div>
            <PinButton id="loksewa-products" />
          </div>
          <span className="mt-6 block text-sm font-medium text-[var(--accent)] transition group-hover:brightness-110">
            {t("hub.loksewaProducts.cta")}
          </span>
        </Link>

        <Link href="/learn/loksewa/ethnic-groups" className={learnHubCardClass}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold tracking-tight text-[var(--text)] group-hover:text-[var(--accent)]">
                {t("hub.loksewaEthnicGroups.title")}
              </h3>
              <p className="mt-1 text-sm text-[var(--muted)]">{t("hub.loksewaEthnicGroups.subtitle")}</p>
            </div>
            <PinButton id="loksewa-ethnic-groups" />
          </div>
          <span className="mt-6 block text-sm font-medium text-[var(--accent)] transition group-hover:brightness-110">
            {t("hub.loksewaEthnicGroups.cta")}
          </span>
        </Link>
      </HubAccordionSection>

      <HubAccordionSection
        sectionId={`${baseId}-technical`}
        titleKey="hub.loksewa.groupTechnical"
        hintKey="hub.loksewa.groupTechnicalHint"
      >
        <Link href="/learn/loksewa/it-officer" className={learnHubCardClass}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold tracking-tight text-[var(--text)] group-hover:text-[var(--accent)]">
                {t("hub.loksewaITOfficer.title")}
              </h3>
              <p className="mt-1 text-sm text-[var(--muted)]">{t("hub.loksewaITOfficer.subtitle")}</p>
            </div>
            <PinButton id="loksewa-it-officer" />
          </div>
          <span className="mt-6 block text-sm font-medium text-[var(--accent)] transition group-hover:brightness-110">
            {t("hub.loksewaITOfficer.cta")}
          </span>
        </Link>
      </HubAccordionSection>
    </div>
  );
}
