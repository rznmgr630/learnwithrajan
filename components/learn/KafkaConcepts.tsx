...
export function KafkaConcepts() {
  const [active, setActive] = useState<KafkaConcept | null>(null);
      ...
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-4xl px-4 pb-20 pt-12 sm:px-6">
          ...
          <div className="relative mb-10">
            ...
            <div className="relative">
              ...
              <p className="mt-4 max-w-xl text-[var(--muted)]">
                {KAFKA_CONCEPT_COUNT} concepts — from event streaming basics and the append-only
                log to partitions, consumer groups, delivery guarantees, log retention, and
                performance tuning. Every concept includes a flow diagram and real Node.js code.
              </p>
            </div>
          </div>
          ...
          <div className="mb-10 flex flex-wrap gap-4">
            {[
              { label: "Concepts", value: `${KAFKA_CONCEPT_COUNT}` },
              { label: "Sections", value: `${KAFKA_SECTIONS.length}` },
              { label: "Code Examples", value: `${KAFKA_CONCEPT_COUNT}` },
            ].map((s) => (
              <div
                key={s.label}
          ...
          <div className="flex flex-col gap-8">
            {KAFKA_SECTIONS.map((section) => {
              const sectionConcepts = KAFKA_CONCEPTS.filter(
                (c) => c.section === section
              );
