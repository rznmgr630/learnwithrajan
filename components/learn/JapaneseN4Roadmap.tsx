...
const weeklyTestIds = [
  ...JAPANESE_N4_WEEKS.filter((w) => w.weeklyTest).map((w) => w.id),
  JAPANESE_N4_FULL_LEVEL_MOCK.id,
];
...
function weeklyTestNeighbour(currentId: string | null, offset: -1 | 1, locale: Locale): LessonNavTarget | null {
  ...
  const test = id ? resolveJapaneseN4WeeklyTestForRoadmap(id) : undefined;
      ...
      <div className="mt-10" id="jp-n4-roadmap-full-mock">
        <div className="rounded-2xl border border-[var(--border)] bg-[color-mix(in_oklab,var(--elevated)_35%,transparent)] p-5 shadow-xl sm:p-6">
          ...
          <h3 className="mt-2 text-lg font-semibold leading-snug text-[var(--text)]">
            <RichText text={pickLocalized(JAPANESE_N4_FULL_LEVEL_MOCK.title, locale)} />
          </h3>
          <p className="mt-1 text-sm leading-relaxed text-[var(--muted)]">
            <RichText text={pickLocalized(JAPANESE_N4_FULL_LEVEL_MOCK.subtitle, locale)} />
          </p>
