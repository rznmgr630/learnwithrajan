...
import { getAllNextjsRoadmapDays, NEXTJS_TOTAL_DAYS } from "@/lib/nextjs-learning/nextjs-challenge-data";
...
export function NextjsRoadmap() {
  ...
  const allDays = useMemo(() => getAllNextjsRoadmapDays(), []);

  return (
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-8 sm:px-6">
      <div className="mb-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-[var(--text)] sm:text-3xl">{t("nextjsRoadmap.title")}</h1>
          <p className="mt-1 text-sm text-[var(--muted)]">{t("nextjsRoadmap.subtitle")}</p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <span className="text-sm text-[var(--muted)]">{t("nextjsRoadmap.overallProgress")}</span>
          <div className="flex items-center gap-3">
            <span className="text-sm tabular-nums text-[var(--text)]">
              {completedCount}/{NEXTJS_TOTAL_DAYS} {t("hub.backend.days")}
            </span>
          </div>
        </div>
        ...
        <p className="mt-2 text-right text-xs text-[var(--muted)]">
          {percent}
          {t("nextjsRoadmap.percentComplete")}
        </p>
      </div>

      <ul className={`mt-8 ${dayGridClass}`} id="nextjs-roadmap-days" aria-label={t("nextjsRoadmap.daysListAria")}>
        {allDays.map((d) => {
      ...
      <div className="mt-14 flex flex-col items-center gap-4" id="nextjs-roadmap-bottom">
        <button
          type="button"
          ...
          onClick={() => document.getElementById("nextjs-roadmap-blurb")?.scrollIntoView({ behavior: "smooth" })}
        >
        ...
        <p id="nextjs-roadmap-blurb" className="max-w-2xl text-center text-sm leading-relaxed text-[var(--muted)]">
          {t("nextjsRoadmap.bottomBlurb")}
        </p>
      </div>
