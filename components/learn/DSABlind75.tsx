...
import { BLIND75_CATEGORIES, BLIND75_TOTAL } from "@/lib/dsa/blind75";
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-5xl px-4 pb-20 pt-12 sm:px-6">
        ...
        <div className="relative mb-10">
          ...
          <div className="relative">
            ...
            <p className="mt-4 max-w-xl text-[var(--muted)]">
              The most frequently asked LeetCode problems in technical interviews. Each problem includes
              the key pattern and a visual mnemonic to help you remember the approach.
            </p>
          </div>
        </div>
              ...
              >
                ...
                <div className="border-t border-[var(--border)]">
                  ...
                  {cat.problems.map((p, i) => (
                    <div
                      key={p.num}
                      className={`grid grid-cols-[2.5rem_1fr_1fr_1fr] gap-x-4 px-4 py-3 text-sm transition hover:bg-[color-mix(in_oklab,var(--elevated)_40%,transparent)] ${i < cat.problems.length - 1 ? "border-b border-[var(--border)]" : ""}`}
                    >
                      {/* # */}
                      ...
                      <div className="flex items-start gap-1.5 min-w-0">
                        <a
                          ...
                          className="font-medium text-[var(--accent)] hover:underline underline-offset-2 leading-snug"
                        >
                          {p.leetcodeNum}. {p.title}
                        </a>
                        {p.premium && (
                          <span className="mt-0.5 shrink-0 text-amber-400" title="Premium">🔒</span>
                        )}
                      </div>
        ...
        <p className="mt-8 text-center text-xs text-[var(--faint)]">
          🔒 Premium problems require a LeetCode subscription. Patterns from the{" "}
          <span className="text-[var(--muted)]">NeetCode Blind 75</span> curated list.
        </p>
      </div>
    </div>
  );
