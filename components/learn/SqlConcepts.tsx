...
function ConceptCard({ concept, onClick }: { concept: SqlConcept; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
...
export function SqlConcepts() {
  const [active, setActive] = useState<SqlConcept | null>(null);
      ...
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-4xl px-4 pb-20 pt-12 sm:px-6">
          ...
          <div className="relative mb-10">
            ...
            <div className="relative">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[var(--accent)]/20 bg-[var(--accent)]/8 px-3 py-1">
                ...
                <span className="text-xs font-medium text-[var(--accent)]">Database Fundamentals</span>
              </div>
              <h1 className="text-4xl font-bold tracking-tight text-[var(--text)] sm:text-5xl">
                Relational Databases
                <br />
                <span className="text-[var(--accent)]">SQL Essentials</span>
              </h1>
              <p className="mt-4 max-w-xl text-[var(--muted)]">
                The {SQL_CONCEPT_COUNT} core concepts — from tables and rows to joins, indexes, and transactions.
              </p>
            </div>
          </div>
          ...
          <div className="mb-10 flex flex-wrap gap-4">
            {[
              { label: "Concepts", value: `${SQL_CONCEPT_COUNT}` },
              { label: "Sections", value: "5" },
              { label: "SQL examples", value: `${SQL_CONCEPT_COUNT}` },
            ].map((s) => (
          ...
          <div className="flex flex-col gap-8">
            {SQL_SECTIONS.map((section) => {
              const sectionConcepts = SQL_CONCEPTS.filter((c) => c.section === section);
