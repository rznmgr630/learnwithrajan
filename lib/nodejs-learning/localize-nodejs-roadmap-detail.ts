...
/** Resolves Node.js roadmap day detail copy for en / np / jp. */
export function localizeNodejsRoadmapDayDetail(detail: RoadmapDayDetail, locale: Locale): ResolvedRoadmapDayDetail {
  const ov = detail.overview;
  const overview =
    ov === undefined ? [] : Array.isArray(ov) ? ov.map((x) => ls(x, locale)) : [ls(ov, locale)];

  const sections = detail.sections?.map((sec) => localizeNodejsSection(sec, locale));
  const faq = detail.faq?.map((item) => localizeNodejsFaq(item, locale));
...
function localizeNodejsSection(sec: RoadmapDayDetailSection, locale: Locale): ResolvedRoadmapDayDetailSection {
  return {
    ...sec,
    title: ls(sec.title, locale),
    items: sec.items?.map((i) => ls(i, locale)),
    blocks: sec.blocks?.map((b) => localizeNodejsBlock(b, locale)),
  };
}

function localizeNodejsBlock(block: RoadmapDetailBlock, locale: Locale): RoadmapDetailBlockResolved {
  switch (block.type) {
    case "paragraph":
...
function localizeNodejsFaq(item: RoadmapDayFaqItem, locale: Locale): ResolvedRoadmapDayFaqItem {
  return {
