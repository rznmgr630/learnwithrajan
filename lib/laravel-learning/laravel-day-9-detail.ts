      ...
      blocks: [
        {
          type: "paragraph",
          text: {
            en: "A <b>session driver</b> is simply the place Laravel stores session data. Same code in your controllers, different storage behind it.\n\n<i>A note on timing: the `database` driver needs a `sessions` table, and tables come from migrations, which is tomorrow on Day 10. Stay on `file` today and switch the driver once migrations feel familiar.</i>",
            np: "Session driver = Laravel session data कहाँ राख्छ। `database` driver लाई migration चाहिन्छ (Day 10)।",
            jp: "セッションドライバは保存先の選択です。`database` ドライバには `sessions` テーブル（Day 10 のマイグレーション）が必要です。",
          },
        },
        {
          type: "table",
...
{
    // Authentication would normally happen here (Day 13).
