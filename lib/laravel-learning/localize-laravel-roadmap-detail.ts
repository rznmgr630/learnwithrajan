...
/** Resolves Laravel roadmap day detail copy for en / np / jp. */
export function localizeLaravelRoadmapDayDetail(detail: RoadmapDayDetail, locale: Locale): ResolvedRoadmapDayDetail {
  const ov = detail.overview;
  const overview =
    ov === undefined ? [] : Array.isArray(ov) ? ov.map((x) => ls(x, locale)) : [ls(ov, locale)];

  const sections = detail.sections?.map((sec) => localizeLaravelSection(sec, locale));
  const faq = detail.faq?.map((item) => localizeLaravelFaq(item, locale));
...
function localizeLaravelSection(sec: RoadmapDayDetailSection, locale: Locale): ResolvedRoadmapDayDetailSection {
  return {
    ...sec,
    title: ls(sec.title, locale),
    items: sec.items?.map((i) => ls(i, locale)),
    blocks: sec.blocks?.map((b) => localizeLaravelBlock(b, locale)),
  };
}

function localizeLaravelBlock(block: RoadmapDetailBlock, locale: Locale): RoadmapDetailBlockResolved {
  switch (block.type) {
    case "paragraph":
...
function localizeLaravelFaq(item: RoadmapDayFaqItem, locale: Locale): ResolvedRoadmapDayFaqItem {
  return {
