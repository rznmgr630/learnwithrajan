...
php artisan route:clear`,
  ...
  project: {
    ...
    brief: "InvoiceHub currently has two routes you wrote by hand, and Blade templates with URLs typed into them. That works at two routes and stops working at twenty.\n\nToday you replace all of it with the routing you have just learned. There is still no database, so the controller keeps returning its hard-coded array. Route model binding has to wait for Day 8, and that is fine: everything else on this list works without it.",
    steps: [
