...
// You build none of these today. Days 3 to 7 build them one at a time.`,
  ...
  project: {
    ...
    brief: "You are going to build one application across this whole track: <b>InvoiceHub</b>, a small invoicing tool. Clients, invoices, line items, payments. Every day from here adds another slice to the same codebase, so by Day 25 you are deploying something you actually built.\n\nToday it does not touch a database. There is no such thing yet. Today's job is narrower and more important: get the thing running, and prove a request can reach code you wrote and come back as a page. Hard-coded data is fine, and you will replace it on Day 10.",
    steps: [
