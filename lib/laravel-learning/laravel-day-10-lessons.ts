...
class AuthController extends Controller
{
    ...
    {
        // Real authentication arrives on Day 17. For now, pretend.
...
Which one, and when
  ...
  database  → queryable, several app servers
              needs a sessions table → a migration → Day 13
...
# SESSION_DRIVER=database    # needs a sessions table (a migration, Day 13)
...
$request->session()->put('user_id', 123);`,
      ...
      keyTakeaways: [
        ...
        "<b>`database`</b> needs a `sessions` table, so it needs a migration, which is Day 13.",
      ...
      quiz: [
        ...
        },
        {
          ...
          correctIndex: 0,
          explanation: "Which is why the switch waits until Day 13.",
        },
        {
...
class AuthController extends Controller
{
    public function login(Request $request)
    {
        // Real authentication is Day 17. This is what it does underneath.
...
// Cookie::queue('api_key', $secret);      // no`,
  ...
  finalQuiz: [
    ...
    },
    {
      ...
      correctIndex: 2,
      explanation: "It writes to a `sessions` table, and tables come from migrations on Day 13.",
    },
    {
