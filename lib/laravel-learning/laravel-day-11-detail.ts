...
export const LARAVEL_DAY_11_DETAIL: RoadmapDayDetail = {
  overview: [
    ...
    {
      en: "Once the app is fast, the next question is who can read it.\n\n• <b>Localization</b> lets you translate your UI into any language using `lang/` files and the `__()` helper\n  ↳ Switch locale at runtime with `App::setLocale('np')`, or set `APP_LOCALE` in `.env` for the default\n• Sessions and flash data are Day 9, so today is only about data you cache and text you translate",
      np: "Localization ले `lang/` files र `__()` द्वारा UI translate गर्छ। Session र flash data Day 9 मा छ।",
      jp: "**ローカライゼーション** は `lang/` と `__()` で UI を多言語化します。セッションとフラッシュデータは Day 9 で扱います。",
    },
  ],
  sections: [
    {
