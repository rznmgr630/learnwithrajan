...
export const LARAVEL_DAY_6_LESSONS: LessonDay = {
  day: 6,
...
enum Status: string
{
    ...
    case Paid  = 'paid';
      ...
      keyTakeaways: [
        ...
        "None of these validate. <b>`integer('abc')` returns `0`</b>, not an error. Validation is Day 7.",
      ],
      commonMistakes: [
...
// return redirect('/new');        302 by default, safe to change later`,
      ...
      keyTakeaways: [
        "Prefer <b>`redirect()->route(...)`</b> over a hard-coded URL, for the same reason as Day 4.",
  ...
  project: {
    ...
    brief: "Day 4 gave InvoiceHub seven resource routes, and most of the methods behind them still just redirect. Today you make them real.\n\nThere is still no database, so keep the hard-coded array in a service and treat it as your store for now. Everything you build today, reading input, handling an upload, redirecting with a flash message, choosing status codes, survives Day 9 unchanged when Eloquent arrives. Validation is tomorrow, so read input directly and accept that it is not yet safe.",
    steps: [
