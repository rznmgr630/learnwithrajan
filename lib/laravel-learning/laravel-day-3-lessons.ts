...
// The controller, route and validation under test are the real ones.`,
  ...
  project: {
    name: "InvoiceHub",
    goal: "Take a payment, without the app knowing who processes it.",
    brief: "An invoice nobody can pay is just a list. Today InvoiceHub starts taking payments.\n\nThe interesting part is not the payment. It is that you will build it so the application never learns which provider it uses. You are writing against a contract, and the container decides at runtime what fulfils it. That is what lets you develop against a fake gateway, run tests without touching a real one, and switch provider later by editing a single line.",
    steps: [
      "Create `app/Contracts/PaymentGateway.php` with one method: `charge(float $amount, string $reference): bool`.",
      "Create `app/Services/Payments/FakePaymentGateway.php` implementing it. Log the charge and return `true`. This is what you develop against.",
      "Create `StripePaymentGateway` next to it. It can return `true` for now. You are practising the wiring, not integrating Stripe.",
      "Add a `gateway` key to `config/invoicing.php` reading `INVOICE_GATEWAY` from `.env`.",
      "Bind the contract in `AppServiceProvider::register()`, picking the implementation based on that config value.",
      "Create `PaymentController` with a `store()` method that type-hints `PaymentGateway`, charges the invoice total, marks the invoice paid, and redirects back with a message.",
      "Add a `POST /invoices/{number}/pay` route and a pay button on the invoice page.",
      "Write an `EnsureInvoiceIsPayable` middleware that blocks payment on an invoice already marked paid, and apply it to that route only.",
      "Flip `INVOICE_GATEWAY` between `fake` and `stripe` and confirm nothing else has to change.",
      "Delete the binding, load the page, and read the `Target [PaymentGateway] is not instantiable` error properly. Then put it back.",
    ],
    acceptance: [
      "Paying an invoice works, and switching gateway is one `.env` change.",
      "`PaymentController` never mentions Stripe or Fake anywhere. It only knows `PaymentGateway`.",
      "Paying an already-paid invoice is stopped by middleware, before the controller runs.",
      "You can explain out loud why deleting the binding produced that error, and why a plain class would not have.",
    ],
    stretch: [
      "Register a small `PaymentLogger` as a `singleton()`, resolve it in two places in one request, and compare `spl_object_id()` on each. Then change it to `bind()` and compare again.",
      "Add a `RefundController` and use contextual binding to give it a different gateway than `PaymentController` gets.",
      "Write a test that binds `FakePaymentGateway`, posts to the pay route, and asserts the charge was recorded. Notice the controller under test is the real one.",
    ],
  },
};
