...
export const LARAVEL_DAY_9_LESSONS: LessonDay = {
  day: 9,
...
foreach ($csvRows as $index => $row) {
    ...
    Invoice::create($validator->validated());
      ...
      keyTakeaways: [
        ...
        "Type-hint it instead of `Request` and both run <b>before your method body</b>, via Day 7's method injection.",
...
class ValidInvoiceNumber implements ValidationRule
{
  ...
  project: {
    ...
    brief: "InvoiceHub accepts anything you type. Today it stops.\n\nThe database still arrives tomorrow, so `unique` and `exists` cannot be tested against real tables yet. Write those rules anyway and comment them with what they will do; everything else on this list works today. The Form Request classes you build now are the ones Day 13 will start saving through.",
    steps: [
