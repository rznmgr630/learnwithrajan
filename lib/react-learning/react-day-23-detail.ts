...
export const REACT_DAY_23_DETAIL: RoadmapDayDetail = {
  overview: [
    {
...
test("Accordion is keyboard accessible", async () => {
  ...
  expect(button).toHaveAttribute("aria-expanded", "true");
    ...
    {
      title: {
        en: "Testing with a real screen reader",
        np: "Real screen reader सँग test गर्ने",
        jp: "実際のスクリーンリーダーでテストする",
      },
      blocks: [
        {
          type: "paragraph",
          text: {
            en: "axe and similar automated tools only catch what can be verified structurally — missing labels, wrong roles, contrast ratios. They cannot tell you whether the experience actually makes sense out loud. Three things only a real screen reader pass reveals:\n• <b>Reading order</b> — CSS can visually reorder elements (flexbox `order`, absolute positioning) while the screen reader still announces them in DOM order, creating a confusing mismatch\n• <b>Announced purpose</b> — an icon button might have a technically valid `aria-label`, but is it actually clear? \"Icon button\" tells a user nothing; \"Delete invoice #1042\" does\n• <b>Dynamic content</b> — does a toast, form error, or loading state actually get announced when it appears, or does it silently update the DOM while the screen reader stays quiet?\n\n<b>Free tools per platform:</b>\n• <b>VoiceOver</b> (macOS/iOS) — built in, no install. Toggle with `Cmd+F5`\n• <b>NVDA</b> (Windows) — free, open source, the most widely used screen reader for testing\n• <b>TalkBack</b> (Android) — built in, for testing mobile web views",
            np: "axe जस्ता automated tools ले structure मात्र check गर्छ — reading order, announced purpose, र dynamic content जस्ता कुरा real screen reader बाट मात्र थाहा हुन्छ। VoiceOver (Mac, Cmd+F5), NVDA (Windows, free) प्रयोग गर्नुहोस्।",
            jp: "axe などの自動ツールは構造的な問題しか検出できません。読み上げ順序・アナウンスの分かりやすさ・動的コンテンツの通知は実際のスクリーンリーダーでしか確認できません。VoiceOver（Mac、Cmd+F5）やNVDA（Windows、無料）を使いましょう。",
          },
        },
        {
          type: "list",
          ...
          items: [
            {
              en: "Turn on VoiceOver/NVDA, then turn your monitor off (or close your eyes) — you should still be able to complete the task by sound alone",
              np: "VoiceOver/NVDA on गरेर monitor off गर्नुहोस् (वा आँखा बन्द) — sound मात्र सुनेर task पूरा गर्न सक्नुपर्छ",
              jp: "VoiceOver/NVDA をオンにしてモニターを消す（または目を閉じる）— 音だけでタスクを完了できるか確認",
            },
            {
              en: "Tab through every interactive element — does each one announce a clear name AND role (\"Submit, button\" not just a beep)?",
              np: "हरेक interactive element मा Tab गर्नुहोस् — नाम र role दुवै clear announce हुन्छ?",
              jp: "すべての操作可能要素を Tab で移動 — 名前とロールが明確にアナウンスされるか？",
            },
            {
              en: "Trigger a toast, error, or loading state — does the screen reader announce it unprompted, without you having to navigate to it?",
              np: "Toast, error, वा loading state trigger गर्नुहोस् — नेभिगेट नगरी नै announce हुन्छ?",
              jp: "トースト・エラー・ローディング状態を発生させる — 移動しなくても自動的にアナウンスされるか？",
            },
            {
              en: "Complete the component's primary task end-to-end using only Tab, Shift+Tab, Enter, Space, and arrow keys — no mouse",
              np: "Mouse नचलाई Tab, Shift+Tab, Enter, Space, arrow keys मात्र प्रयोग गरी main task पूरा गर्नुहोस्",
              jp: "マウスを使わず Tab・Shift+Tab・Enter・Space・矢印キーだけで主要タスクを完了できるか",
            },
          ],
        },
      ],
    ...
    {
      title: {
        en: "Color contrast — legible text and UI for low-vision users",
        np: "Color contrast — low-vision users को लागि legible UI",
        jp: "カラーコントラスト — 弱視ユーザーのための可読性",
      },
      blocks: [
        {
          type: "paragraph",
          text: {
            en: "WCAG AA requires a minimum contrast ratio between text/UI and its background:\n• <b>4.5:1</b> for normal text\n• <b>3:1</b> for large text (18pt+/24px+, or 14pt+/18.5px+ bold) and for UI components like input borders and icon buttons\n\n<b>Why this bug is so easy to introduce:</b> design systems often lean on light grays for a \"soft\" aesthetic — placeholder text, disabled states, secondary captions. A gray that looks fine to a designer on a calibrated monitor can fail contrast badly for users with low vision, in bright sunlight, or on a cheap screen. Low-opacity text (`opacity-50` on a dark color) is a common repeat offender since the effective contrast depends on the background it's composited against.\n\n<b>How to check it:</b> Chrome DevTools' color picker (click any color swatch in the Styles panel) shows the contrast ratio live against the background and flags AA/AAA pass or fail as you adjust. Browser extensions (e.g. \"WCAG Color contrast checker\") do a full-page sweep.",
            np: "WCAG AA: normal text 4.5:1, large text/UI 3:1 contrast चाहिन्छ। Light gray text वा low-opacity disabled states मा contrast bug सजिलै आउँछ। Chrome DevTools को color picker ले live contrast ratio देखाउँछ।",
            jp: "WCAG AA では通常テキスト4.5:1、大きいテキスト/UIは3:1のコントラスト比が必要です。淡いグレーや低不透明度のテキストはコントラスト不足になりがちです。Chrome DevTools のカラーピッカーでライブ確認できます。",
          },
        },
        {
          type: "code",
          title: { en: "Contrast failure vs fix", np: "Contrast failure vs fix", jp: "コントラスト不良と修正例" },
          code: `/* ❌ FAILS AA — light gray on white, ratio ≈ 2.9:1 */
.caption {
  color: #a3a3a3;      /* neutral-400 */
  background: #ffffff;
}

/* ✅ PASSES AA — darker gray on white, ratio ≈ 4.6:1 */
.caption {
  color: #6b6b6b;      /* roughly neutral-600 */
  background: #ffffff;
}

/* ❌ FAILS — disabled button text via opacity alone can drop below 3:1
   depending on what's behind it */
.button-disabled {
  color: #1f2937;
  opacity: 0.4;         /* effective contrast is unpredictable */
}

/* ✅ Prefer a fixed, pre-computed color over opacity for disabled UI text */
.button-disabled {
  color: #9ca3af;       /* a specific gray verified to meet 3:1 for UI components */
  background: #f3f4f6;
}`,
        },
      ],
