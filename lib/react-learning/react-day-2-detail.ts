  ...
  overview: [
    {
      ...
      jp: "2日目は**開発環境とプロジェクトの骨格**です。エディタと Node をそろえ、Vite でアプリを生成し、ディレクトリの役割を把握し、小さなコンポーネントを切り出してみます。",
    },
    {
      ...
      jp: "**03〜07**：作成・構成・コンポーネント → **React が UI をどう更新するか**（レンダーとコミット、和解の概要）→ 核のまわりの**エコシステム**（ルーティング、取得、スタイル、メタフレームワーク）。**Vite** 前提。CRA は新規では旧式寄りですが考え方は共通です。",
    },
  ],
  sections: [
    {
      ...
      blocks: [
        {
          ...
          text: {
            ...
            jp: "**関数コンポーネント**は先頭大文字の関数で **JSX を返す**ものです。ファイルごとに主役が一つなら **default export**、複数なら **named export** が使われます。",
          },
        },
        {
          type: "code",
      ...
      blocks: [
        {
          ...
          text: {
            ...
            jp: "大ざっぱに **レンダー** と **コミット** があります。**レンダー**で関数コンポーネントが走り、UI の**記述**（**要素**のツリー。DOM ではない）が作られます。**コミット**で実 DOM へ必要最小限の変更が入ります。",
          },
        },
        {
          type: "list",
        ...
        },
        {
          ...
          text: {
            ...
            jp: "**仮想 DOM** とは、ざっくり **メモリ上の React 要素ツリー**（前回レンダーと今回レンダー）を指す言い方です。ブラウザの DOM を常に丸ごと二重管理するのではなく、**要素の記述**同士を比較し、必要な差分だけ **実 DOM** に反映します。まとめて更新するので、細かい state 変更が必ずしもレイアウト嵐に直結しません。",
          },
        },
        {
          type: "diagram",
      ...
      blocks: [
        {
          ...
          text: {
            ...
            jp: "**`react` / `react-dom`** は UI の組み立てと更新が中心です。**ルーティング**、**サーバデータの取得・キャッシュ**、**フォーム**、**スタイル**、**テスト**などは周辺の**エコシステム**か、それらを束ねる**メタフレームワーク**（Next.js、Remix など）で補います。",
          },
        },
        {
          type: "table",
  ...
  faq: [
    ...
    },
    {
      ...
      answer: {
        ...
        jp: "ページ全体の HTML を丸ごと複製しているわけではありません。**要素を表す軽いオブジェクトのツリー**で、ブラウザの **実 DOM は別**です。React はそのツリー同士の差分から、実 DOM への**最小の更新**を計算します。",
      },
    },
    {
      question: {
      ...
      answer: {
        ...
        jp: "**Next.js は React 上のフレームワーク**です。コンポーネントは React のままですが、ルーティングやビルド、データ取得の置き場（クライアント／サーバ）の扱いが強くなります。",
      },
    },
  ],
  bullets: [
