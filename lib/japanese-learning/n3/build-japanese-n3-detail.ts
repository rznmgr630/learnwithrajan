...
import { twentyN3KanjiForDay } from "@/lib/japanese-learning/n3/n3-kanji-pool";
import {
  youtubeClipsForN3Lesson,
  youtubeClipsForN3SprintDay,
} from "@/lib/japanese-learning/n3/n3-youtube-links";
import type {
  JapaneseDetailBlock,
...
export function buildJapaneseN3DayDetail(
  day: number,
  ...
  const youtubeVideos =
    spec.minnaLesson !== null
      ? youtubeClipsForN3Lesson(spec.minnaLesson)
      : youtubeClipsForN3SprintDay(day);

  const kanjiItems = twentyN3KanjiForDay(day);
  ...
  const kanjiIntro: LocalizedString = {
    en: `Practice writing each character on grid paper while tracing stroke order. N3 kanji appear frequently in JLPT reading passages — accuracy matters.`,
    np: `रेखाहरू पछ्याउँदै ग्रिड कागजमा प्रत्येक वर्ण लेख्न अभ्यास गर्नुहोस्। N3 कांजी JLPT पढाइ अनुच्छेदमा बारम्बार आउँछन् — शुद्धता महत्त्वपूर्ण छ।`,
    jp: `方眼紙に筆順をなぞりながら各字を書いて練習してください。N3の漢字はJLPT読解に頻出です——正確さが大切。`,
  };
  const kanjiCaption: LocalizedString = {
    en: `Twenty N3 kanji for Day ${day} · tap "Open stroke SVG" if an image fails to load`,
    np: `दिन ${day} का बीस N3 कांजी · छवि लोड नभएमा Open stroke SVG ट्याप गर्नुहोस्`,
    jp: `Day ${day} のN3漢字20字 · 画像が出ないときは「Open stroke SVG」を開いてください`,
  };
  ...
  return {
    ...
    bullets: [
      ...
      {
        en: `Copy every N3 kanji stroke SVG row onto paper once.`,
        np: `प्रत्येक N3 कांजी रेखा SVG पङ्क्ति कागजमा एक पटक नक्कल गर्नुहोस्।`,
        jp: `N3漢字の筆順SVGの行をすべて紙に一度ずつ写す。`,
      },
      {
