/**
 * Shared helpers for JLPT N3-style weekly papers (five tests × 20 MCQs each).
 */
...
export const N3_YT_SAMPLE_CLIP_ID = "dPjxIuJZiZc";
...
export function clipsN3(...lessons: number[]): { url: string; title: string }[] {
  const out: { url: string; title: string }[] = [];
  for (const n of lessons) {
    for (const c of youtubeClipsForN3Lesson(n)) {
...
export function n3ListeningIntro(
  id: string,
  scenario: string,
  instruction: string,
  lessonNums: number[],
  ...
  const base = [...clipsN3(...lessonNums)];
  if (sprintDay !== undefined) {
    for (const c of youtubeClipsForN3SprintDay(sprintDay)) {
  ...
  return {
    ...
    embedVideoId: N3_YT_SAMPLE_CLIP_ID,
  };
}
...
/**
 * Builds five JLPT N3-style papers (Test 1–5) from parallel pools.
 * Test 1 is easiest; Test 5 is hardest.
 */
export function buildN3FivePaperWeek(options: {
  weekPrefix: string;
