...
git show HEAD`,
        },
        {
          ...
          text: {
            ...
            jp: "`git commit --amend` で直近のコミットを置き換えます。メッセージの誤字修正や忘れたファイルの追加に使います。**push 前限定**です。公開後に使うと他者のリポとの不整合が生じます。",
          },
        },
        {
          type: "code",
      ...
      blocks: [
        {
          ...
          text: {
            ...
            jp: "ローカルリポはあなたのマシンにしかありません。PCが壊れたら全コミットが消えます。リモートは**バックアップ**と**共同作業**の2つを同時に解決します。リモートなしでは手動でファイルをやり取りするしかありません。",
          },
        },
        {
          type: "list",
          ...
          items: [
            {
              ...
              jp: "reflog は**ローカルのみ**です。リモートとは共有されず、新しい clone には含まれません。",
            },
            {
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            [
              ...
              { en: "Restore a specific stash but **keep it in the stash list**", np: "विशेष stash पुनर्स्थापित गर्छ तर **सूचीमा राख्छ**", jp: "特定の stash を復元するが**リストには残す**" },
          ...
          items: [
            {
              ...
              jp: "スクリプトは**決定的**である必要があります。同じコミットで pass/fail が揺れると bisect が誤った答えを出します。まず手動で数回実行して確認してください。",
            },
            {
              ...
              jp: "終了コード `125` は `bisect run` にそのコミットを**スキップ**させるシグナルです（例：コンパイル不能）。その周辺の探索は続きます。",
            },
          ],
        },
      ],
