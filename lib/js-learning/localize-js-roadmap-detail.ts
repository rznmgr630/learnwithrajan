...
/** Resolves JS roadmap day detail copy for en / np / jp. */
export function localizeJsRoadmapDayDetail(detail: RoadmapDayDetail, locale: Locale): ResolvedRoadmapDayDetail {
  const ov = detail.overview;
  const overview =
    ov === undefined ? [] : Array.isArray(ov) ? ov.map((x) => ls(x, locale)) : [ls(ov, locale)];

  const sections = detail.sections?.map((sec) => localizeJsSection(sec, locale));
  const faq = detail.faq?.map((item) => localizeJsFaq(item, locale));
...
function localizeJsSection(sec: RoadmapDayDetailSection, locale: Locale): ResolvedRoadmapDayDetailSection {
  return {
    ...sec,
    title: ls(sec.title, locale),
    items: sec.items?.map((i) => ls(i, locale)),
    blocks: sec.blocks?.map((b) => localizeJsBlock(b, locale)),
  };
}

function localizeJsBlock(block: RoadmapDetailBlock, locale: Locale): RoadmapDetailBlockResolved {
  switch (block.type) {
    case "paragraph":
...
function localizeJsFaq(item: RoadmapDayFaqItem, locale: Locale): ResolvedRoadmapDayFaqItem {
  return {
