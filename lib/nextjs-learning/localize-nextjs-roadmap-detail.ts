...
export function localizeNextjsRoadmapDayDetail(detail: RoadmapDayDetail, locale: Locale): ResolvedRoadmapDayDetail {
  const ov = detail.overview;
  const overview =
    ov === undefined ? [] : Array.isArray(ov) ? ov.map((x) => ls(x, locale)) : [ls(ov, locale)];

  const sections = detail.sections?.map((sec) => localizeNextjsSection(sec, locale));
  const faq = detail.faq?.map((item) => localizeNextjsFaq(item, locale));
...
function localizeNextjsSection(sec: RoadmapDayDetailSection, locale: Locale): ResolvedRoadmapDayDetailSection {
  return {
    ...sec,
    title: ls(sec.title, locale),
    items: sec.items?.map((i) => ls(i, locale)),
    blocks: sec.blocks?.map((b) => localizeNextjsBlock(b, locale)),
  };
}

function localizeNextjsBlock(block: RoadmapDetailBlock, locale: Locale): RoadmapDetailBlockResolved {
  switch (block.type) {
    case "paragraph":
...
function localizeNextjsFaq(item: RoadmapDayFaqItem, locale: Locale): ResolvedRoadmapDayFaqItem {
  return {
