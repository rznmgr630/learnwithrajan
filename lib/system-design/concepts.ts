...
export const SYSTEM_DESIGN_CONCEPTS: SystemDesignConcept[] = [
  ...
  },
  {
    ...
    description:
      "Every system is mainly either data-intensive or compute-intensive.\n\nData-intensive systems spend most of their time storing, retrieving, and moving data. The CPU is usually not the main bottleneck. Examples include Instagram feeds, WhatsApp messages, banking transactions, and analytics dashboards. Performance is improved through better database design, caching, replication, sharding, indexing, and optimized queries.\n\nCompute-intensive systems spend most of their time performing calculations. The database is usually not the bottleneck. Examples include video rendering, image processing, machine learning training, flight simulators, and cryptography. Performance is improved by adding more CPU or GPU power, using parallel processing, and optimizing algorithms.\n\nA simple way to tell the difference is: if the system is slow because it's waiting for data to be stored, fetched, or transferred, it's data-intensive. If the system is slow because it's busy performing calculations, it's compute-intensive.\n\nMany real-world applications contain both. For example, YouTube is data-intensive when serving videos to users, but compute-intensive when generating video recommendations. Understanding which part of the system is the bottleneck helps you choose the right scaling and optimization strategy.",
    whyItMatters:
