...
import { NEXTJS_TOTAL_DAYS, seedNextjsCompletedDayNumbers } from "@/lib/nextjs-learning/nextjs-challenge-data";

const STORAGE_KEY = "learnwithrajan.nextjs.completed";
const LOCAL_EVENT = "learnwithrajan.nextjs.changed";
...
function deserialize(raw: string): Set<number> | null {
  try {
    ...
    return new Set(
      arr.filter((n): n is number => typeof n === "number" && n >= 1 && n <= NEXTJS_TOTAL_DAYS),
    );
  } catch {
    return null;
  }
...
function getServerSnapshot(): string {
  return serialize(seedNextjsCompletedDayNumbers());
}
...
export function useNextjsProgress() {
  ...
  const percent = useMemo(
    () => Math.round((Math.min(completedCount, NEXTJS_TOTAL_DAYS) / NEXTJS_TOTAL_DAYS) * 100),
    [completedCount],
  );
  ...
  const toggleDay = useCallback((day: number) => {
    ...
    const raw = window.localStorage.getItem(STORAGE_KEY);
    const base =
      raw === null || raw === "" ? seedNextjsCompletedDayNumbers() : deserialize(raw) ?? seedNextjsCompletedDayNumbers();
