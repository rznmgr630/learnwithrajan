...
import { DEVOPS_TOTAL_DAYS, seedDevopsCompletedDayNumbers } from "@/lib/devops-learning/devops-challenge-data";

const STORAGE_KEY = "learnwithrajan.devops91.completed";
const LOCAL_EVENT = "learnwithrajan.devops91.changed";
...
function deserialize(raw: string): Set<number> | null {
  try {
    ...
    return new Set(
      arr.filter((n): n is number => typeof n === "number" && n >= 1 && n <= DEVOPS_TOTAL_DAYS),
    );
  } catch {
    return null;
  }
...
function getServerSnapshot(): string {
  return serialize(seedDevopsCompletedDayNumbers());
}
...
export function useDevopsProgress() {
  ...
  const percent = useMemo(
    () => Math.round((Math.min(completedCount, DEVOPS_TOTAL_DAYS) / DEVOPS_TOTAL_DAYS) * 100),
    [completedCount],
  );
  const toggleDay = useCallback((day: number) => {
    ...
    const raw = window.localStorage.getItem(STORAGE_KEY);
    const base =
      raw === null || raw === "" ? seedDevopsCompletedDayNumbers() : deserialize(raw) ?? seedDevopsCompletedDayNumbers();
