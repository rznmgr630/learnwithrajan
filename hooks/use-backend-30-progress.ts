...
import { seedCompletedDayNumbers, TOTAL_DAYS } from "@/lib/challenge-data";

const STORAGE_KEY = "learnwithrajan.backend30.completed";
const LOCAL_EVENT = "learnwithrajan.backend30.changed";
...
function deserialize(raw: string): Set<number> | null {
  try {
    ...
    return new Set(
      arr.filter((n): n is number => typeof n === "number" && n >= 1 && n <= TOTAL_DAYS),
    );
  } catch {
    return null;
  }
...
function getServerSnapshot(): string {
  return serialize(seedCompletedDayNumbers());
}
...
export function useBackend30Progress() {
  ...
  const percent = useMemo(
    () => Math.round((Math.min(completedCount, TOTAL_DAYS) / TOTAL_DAYS) * 100),
    [completedCount],
  );
  ...
  const toggleDay = useCallback((day: number) => {
    ...
    const base =
      raw === null || raw === ""
        ? seedCompletedDayNumbers()
        : deserialize(raw) ?? seedCompletedDayNumbers();
