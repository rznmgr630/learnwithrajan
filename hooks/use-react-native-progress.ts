...
import {
  ...
  seedReactNativeCompletedDayNumbers,
} from "@/lib/react-native-learning/react-native-challenge-data";

const STORAGE_KEY = "learnwithrajan.react-native.completed";
const LOCAL_EVENT = "learnwithrajan.react-native.changed";
...
function deserialize(raw: string): Set<number> | null {
  try {
    ...
    return new Set(
      arr.filter((n): n is number => typeof n === "number" && n >= 1 && n <= REACT_NATIVE_TOTAL_DAYS),
    );
  } catch {
    return null;
  }
...
function getServerSnapshot(): string {
  return serialize(seedReactNativeCompletedDayNumbers());
}
...
export function useReactNativeProgress() {
  ...
  const percent = useMemo(
    () => Math.round((Math.min(completedCount, REACT_NATIVE_TOTAL_DAYS) / REACT_NATIVE_TOTAL_DAYS) * 100),
    [completedCount],
  );
  ...
  const toggleDay = useCallback((day: number) => {
    ...
    const base =
      raw === null || raw === ""
        ? seedReactNativeCompletedDayNumbers()
        : (deserialize(raw) ?? seedReactNativeCompletedDayNumbers());
