...
import { JS_TOTAL_DAYS, seedJsCompletedDayNumbers } from "@/lib/js-learning/js-challenge-data";

const STORAGE_KEY = "learnwithrajan.js.completed";
const LOCAL_EVENT = "learnwithrajan.js.changed";
...
function deserialize(raw: string): Set<number> | null {
  try {
    ...
    return new Set(
      arr.filter((n): n is number => typeof n === "number" && n >= 1 && n <= JS_TOTAL_DAYS),
    );
  } catch {
    return null;
  }
...
function getServerSnapshot(): string {
  return serialize(seedJsCompletedDayNumbers());
}
...
export function useJsProgress() {
  ...
  const percent = useMemo(
    () => Math.round((Math.min(completedCount, JS_TOTAL_DAYS) / JS_TOTAL_DAYS) * 100),
    [completedCount],
  );
  ...
  const toggleDay = useCallback((day: number) => {
    ...
    const raw = window.localStorage.getItem(STORAGE_KEY);
    const base =
      raw === null || raw === "" ? seedJsCompletedDayNumbers() : deserialize(raw) ?? seedJsCompletedDayNumbers();
