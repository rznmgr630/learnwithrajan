...
import { LARAVEL_TOTAL_DAYS, seedLaravelCompletedDayNumbers } from "@/lib/laravel-learning/laravel-challenge-data";

const STORAGE_KEY = "learnwithrajan.laravel.completed";
const LOCAL_EVENT = "learnwithrajan.laravel.changed";
...
function getServerSnapshot(): string {
  return serialize(seedLaravelCompletedDayNumbers());
}
...
export function useLaravelProgress() {
  ...
  const toggleDay = useCallback((day: number) => {
    ...
    const raw = window.localStorage.getItem(STORAGE_KEY);
    const base =
      raw === null || raw === "" ? seedLaravelCompletedDayNumbers() : deserialize(raw) ?? seedLaravelCompletedDayNumbers();
